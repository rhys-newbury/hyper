...
class MLPGenerator(nn.Module):
    def __init__(
        ...
        # Takes noise point on sphere as additional input now
        in_dim = noise_dim + class_emb_dim + omega_emb_dim
        layers = []
        for _ in range(num_layers):
            layers.extend([nn.Linear(in_dim, hidden_dim), nn.SiLU(inplace=True)])
    ...
    def forward(self, noise, class_labels, omega):
        """
        noise:        [B, noise_dim] — points on S^(D-1), normalized
        class_labels: [B] int64
        omega:        [B] float
        returns:      [B, latent_dim] — points on S^(D-1)
        """
        b = noise.shape[0]
        cls = class_labels.clone()
        cls = torch.where(cls < 0, torch.full_like(cls, self.num_classes), cls)
        ...
        inp = torch.cat([noise, c_emb, o_emb], dim=-1)
        v_raw = self.mlp(inp)                                      # [B, D] unconstrained

        # Project onto tangent space at noise: remove component along noise
        v = v_raw - (v_raw * noise).sum(dim=-1, keepdim=True) * noise  # [B, D]

        # Exp map: walk from noise along v
        v_norm = v.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        return torch.cos(v_norm) * noise + torch.sin(v_norm) * (v / v_norm)
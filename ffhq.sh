...
for EPS in 0.1 1.0 10.0; do
  ...
  python -m ffhq.reinforce_sinkh_ffhq \
    --train-npz $TRAIN_NPZ \
    --test-npz $TEST_NPZ \
    --save-path runs/ffhq/eps_${TAG}_sinkhorn_reinforce/drift_ffhq_model.pt \
    --emd-plot runs/ffhq/eps_${TAG}_sinkhorn_reinforce/drift_ffhq_emd.png \
    --emd-perclass-plot runs/ffhq/eps_${TAG}_sinkhorn_reinforce/drift_ffhq_emd_perclass.png \
    --pca-plot runs/ffhq/eps_${TAG}_sinkhorn_reinforce/drift_ffhq_pca.png \
    --d-z 512 --d-e 64 --hidden 1024 --n-hidden 3 \
